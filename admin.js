...
function openProductEditor(id) {
  ...
  editor.innerHTML = `
    ...
    <div class="ap-editor-panel">
      ...
      <div class="ape-grid">
        ...
        <div>
          <label class="ape-label">BASE PRICE (USD per day · auto-fill)</label>
          <input class="ape-input" id="fBase" type="number" step="0.5" min="0" value="${p.base}"/>
          <div class="ape-tier-preview" id="fTierPreview"></div>
        </div>
        <div>
        ...
        <div class="ape-full">
          <label class="ape-label">PRICES · PER TIER (USD)</label>
          <p class="ape-hint">Set explicit price per duration. Leave empty → auto-compute from base × multiplier. Set <code>0</code> → tier hidden from customers.</p>
          <div style="display:grid;grid-template-columns:repeat(5, 1fr);gap:8px">
            ${Store.TIERS.map(t => `
              <div>
                <label class="ape-label" style="text-align:center">${t.dur.toUpperCase()}</label>
                <input class="ape-input" data-tier-price="${escapeHtml(t.dur)}" type="number" min="0" step="0.5" placeholder="auto" value="${(p.prices && p.prices[t.dur] !== undefined) ? p.prices[t.dur] : ""}" style="text-align:center"/>
              </div>
            `).join("")}
          </div>
        </div>
        ...
        <div class="ape-full">
          <label class="ape-label">PAYMENT LINKS · PER TIER</label>
          <p class="ape-hint">Direct checkout URL per duration. Leave empty to fall back to <code>Settings → Payment Links</code> templates.</p>
          ${Store.TIERS.map((t, i) => `
            <div style="display:grid;grid-template-columns:90px 1fr;gap:8px;align-items:center;margin-bottom:6px">
              <label class="ape-label" style="margin:0;color:var(--accent-soft)">${t.dur.toUpperCase()}</label>
              <input class="ape-input" data-tier-link="${escapeHtml(t.dur)}" placeholder="https://buy.example.com/..." value="${escapeHtml((p.buyLinks && p.buyLinks[t.dur]) || "")}"/>
            </div>
          `).join("")}
        </div>
  ...
  function refreshTierPreview() {
    const base = Number($("#fBase", editor).value) || 0;
    const explicit = {};
    $$("[data-tier-price]", editor).forEach(inp => {
      const v = inp.value.trim();
      if (v !== "") explicit[inp.dataset.tierPrice] = Number(v);
    });
    const tiers = Store.makePrices({ base, prices: explicit });
    $("#fTierPreview", editor).innerHTML = tiers
      .map(t => t.price > 0
        ? `<span><b>${t.dur}</b> $${t.price}</span>`
        : `<span style="opacity:.4;text-decoration:line-through"><b>${t.dur}</b> hidden</span>`)
      .join(" · ");
  }
  refreshTierPreview();
  $("#fBase", editor).addEventListener("input", refreshTierPreview);
  $$("[data-tier-price]", editor).forEach(inp => inp.addEventListener("input", refreshTierPreview));
  ...
  $("#fSave", editor).addEventListener("click", () => {
    const newP = {
      ...
      prices: (function() {
        ...
        $$("[data-tier-price]", editor).forEach(inp => {
          const v = inp.value.trim();
          if (v !== "") out[inp.dataset.tierPrice] = Number(v);
        });
        return out;
      })(),
      buyLinks: (function() {
        const links = {};
        $$("[data-tier-link]", editor).forEach(inp => {
          const v = inp.value.trim();
          if (v) links[inp.dataset.tierLink] = v;
        });
        return links;
      })(),
    };
