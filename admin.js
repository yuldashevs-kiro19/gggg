...
function openProductEditor(id) {
  ...
  $("#fSave", editor).addEventListener("click", () => {
    // Pull fresh existing record so we don't accidentally drop any field
    // (reviews, custom metadata, etc) that the form doesn't render.
    const existing = isNew ? {} : (Store.getProduct(p.id) || {});
    const newP = {
      ...existing,
      ...
      // Preserve existing reviews — never overwrite from the editor (it doesn't show them)
      reviews: (existing.reviews || p.reviews || []),
    ...
    // Guard against ID collision with another product
    if (isNew) {
      const existingById = Store.getProducts().find(x => x.id === newP.id);
      if (existingById) {
        showToast(`ID ${newP.id} already exists — pick a different ID`, "ERROR", "err");
        return;
      }
    }
